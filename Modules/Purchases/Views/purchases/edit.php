...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function addProductRow(product, isExisting, price) {
            ...
            const displayName = product.variation_product_name || product.product_name || 'Unnamed Product';
        ...
        <td><input type="number" name="products[${uniqueIndex}][price]" value="${validPrice.toFixed(2)}" class="form-control price-input" /></td>
    ...
    `;
            ...
            fetch('<?= site_url(route_to('purchases.search')) ?>?query=' + encodeURIComponent(query))
                ...
                .then(data => {
                    ...
                    data.forEach(product => {
                        ...
                        // Exclude single products already added; allow variations of existing products
                        if (addedProductNames.has(product.product_name) && !product.has_variation) {
                            return; // Skip adding this single product to suggestions
                        }
        ...
        productSuggestions.addEventListener('click', function (e) {
            if (e.target.classList.contains('list-group-item') && e.target.getAttribute('data-product')) {
                ...
                const productName = product.variation_product_name || product.product_name || 'Unnamed Product';
